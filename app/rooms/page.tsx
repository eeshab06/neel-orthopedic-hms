...
function buildBillHTML(bill: any): string {
  ...
  const medRows = bill.medicinesList?.length > 0
    ? bill.medicinesList.map((m: any, i: number) => `
        <tr>
          <td style="padding:5px 8px;border-bottom:1px solid #eee;font-size:11px;">${String(i+1).padStart(2,"0")}</td>
          <td style="padding:5px 8px;border-bottom:1px solid #eee;font-size:11px;">${m.name}${m.notes ? ` (${m.notes})` : ""}</td>
          <td style="padding:5px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:center;">${m.quantity}</td>
          <td style="padding:5px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:right;">₹${(m.unit_price||0).toFixed(2)}</td>
          <td style="padding:5px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:right;font-weight:600;">₹${((m.unit_price||0)*m.quantity).toFixed(2)}</td>
        </tr>`).join("")
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;color:#000;}
  ...
  body{font-family:Arial,sans-serif;font-size:11px;line-height:1.45;background:#fff;}
  hr{border:none;border-top:1.5px solid #000;margin:5px 0;}
  table{width:100%;border-collapse:collapse;}
  .sh{font-weight:bold;font-size:12px;text-align:center;text-decoration:underline;margin:8px 0 6px;letter-spacing:1px;}
  .lb{font-weight:bold;}
  .ab{background:#f5f5f5;border:1px solid #ccc;padding:8px 12px;text-align:right;margin-top:6px;}
  .fn{text-align:center;font-size:9px;color:#666;margin-top:8px;border-top:1px solid #ccc;padding-top:5px;font-style:italic;}
</style></head><body>
<table style="margin-bottom:6px;"><tr>
  ...
  <td style="width:30%;text-align:right;vertical-align:top;">
    <div style="font-size:10px;">Date: ${today}</div>
    <div style="font-size:10px;">Bill No: IPD-${Date.now().toString().slice(-6)}</div>
  </td>
</tr></table>
<hr/>
<table style="margin:5px 0;"><tr>
  <td style="width:50%;vertical-align:top;padding-right:16px;">
    <div><span class="lb">Patient Name:</span> ${patientName}</div>
    <div><span class="lb">UHID:</span> ${uhid}</div>
    <div><span class="lb">Contact:</span> ${phone}</div>
    <div><span class="lb">Consulting Doctor:</span> Dr. G.K. Boob (DNB Ortho)</div>
  </td>
  <td style="width:50%;vertical-align:top;">
    <div><span class="lb">Admission Date:</span> ${admitDate}</div>
    <div><span class="lb">Discharge Date:</span> ${today}</div>
    <div><span class="lb">Duration:</span> ${bill.days} Day${bill.days>1?"s":""}</div>
    <div><span class="lb">Room No:</span> ${bill.room.room_number} (${bill.room.room_type})</div>
  </td>
</tr></table>
<hr/>
<div class="sh">PROVISIONAL BILL</div>
<table style="margin-bottom:8px;">
  <thead><tr style="background:#f0f0f0;">
    <th style="padding:6px 8px;text-align:left;border-bottom:1.5px solid #000;">Code</th>
    <th style="padding:6px 8px;text-align:left;border-bottom:1.5px solid #000;">Particulars</th>
    <th style="padding:6px 8px;text-align:right;border-bottom:1.5px solid #000;">Amount (₹)</th>
  </tr></thead>
  <tbody>
...
<div class="ab">
  <table style="width:280px;margin-left:auto;">
    <tr><td style="font-size:11px;padding:2px 0;">Total Bill Amount:</td><td style="font-size:11px;text-align:right;font-weight:bold;">₹${bill.total.toFixed(2)}</td></tr>
    <tr><td style="font-size:11px;padding:2px 0;">Amount Payable:</td><td style="font-size:11px;text-align:right;font-weight:bold;">₹${bill.total.toFixed(2)}</td></tr>
    <tr><td style="font-size:11px;padding:2px 0;">Amount Paid:</td><td style="font-size:11px;text-align:right;">₹0.00</td></tr>
    <tr style="border-top:1px solid #000;"><td style="font-size:12px;padding:3px 0;font-weight:bold;">Balance:</td><td style="font-size:12px;text-align:right;font-weight:bold;">₹${bill.total.toFixed(2)}</td></tr>
  </table>
  <div style="font-size:10px;margin-top:4px;text-align:right;"><span style="font-weight:bold;">Amount in words:</span> ${totalInWords(Math.round(bill.total))}</div>
</div>
...
<table style="margin:4px 0;"><tr style="background:#f0f4ff;">
  <td style="padding:8px;font-size:14px;font-weight:bold;">GRAND TOTAL</td>
  <td style="padding:8px;font-size:16px;font-weight:800;text-align:right;">₹${bill.total.toFixed(2)}</td>
</tr></table>
...
export default function RoomsPage() {
  ...
  const handlePrintBill = () => {
    ...
    pw.document.write(buildBillHTML(bill));
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff", fontFamily: "Georgia, serif" }}>
      ...
      <div style={{ padding: "24px 5%" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "24px" }}>
          {[{ label: "Total Rooms", value: stats.total, color: "#0a2463" }, { label: "Occupied", value: stats.occupied, color: "#dc2626" }, { label: "Available", value: stats.available, color: "#16a34a" }, { label: "ICU Occupied", value: stats.icu, color: "#f59e0b" }].map((s, i) => (
        ...
        {loading ? <div style={{ textAlign: "center", padding: "60px", color: "#666" }}>Loading rooms...</div> : (
            ...
            {filteredRooms.map(room => (
              <div key={room.room_id} style={{ background: getRoomColor(room), borderRadius: "16px", padding: "24px", border: `2px solid ${getRoomBorder(room)}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                  ...
                  <div style={{ padding: "4px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: "700", background: room.is_occupied ? "#fee2e2" : "#dcfce7", color: room.is_occupied ? "#dc2626" : "#16a34a", border: `1px solid ${room.is_occupied ? "#fca5a5" : "#bbf7d0"}` }}>
                    {room.is_occupied ? "Occupied" : "Available"}
                  </div>
                </div>
                <div style={{ fontSize: "13px", color: "#666", marginBottom: "12px" }}>₹{room.price_per_day.toLocaleString()}/day + ₹{room.doctor_charges_per_day.toLocaleString()} doctor</div>
                {room.is_occupied && room.patient && (
                  <div style={{ background: "white", borderRadius: "10px", padding: "12px", marginBottom: "12px", border: "1px solid rgba(0,0,0,0.06)" }}>
                    ...
                    {room.admitted_on && <div style={{ color: "#888", fontSize: "12px", marginTop: "4px" }}>Day {getDaysSince(room.admitted_on)} · Admitted {new Date(room.admitted_on).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>}
                  </div>
                )}
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  {!room.is_occupied ? (
                    <button onClick={() => { setSelectedRoom(room); setShowAssign(true); }} style={{ flex: 1, background: "#0a2463", color: "white", border: "none", padding: "10px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>Assign Patient</button>
                  ) : (
                    <>
                      <button onClick={() => openMedicinesModal(room)} style={{ flex: 1, background: "#7c3aed", color: "white", border: "none", padding: "10px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>💊 Medicines</button>
                      <button onClick={() => { setSelectedRoom(room); calculateBill(room); }} style={{ flex: 1, background: "#1a73e8", color: "white", border: "none", padding: "10px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>View Bill</button>
                      <button onClick={() => { setSelectedRoom(room); calculateBill(room); setShowDischarge(true); }} style={{ width: "100%", background: "#dc2626", color: "white", border: "none", padding: "10px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer", marginTop: "4px" }}>Discharge</button>
                    </>
                  )}
                </div>
              </div>
      ...
      {showAssign && selectedRoom && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "20px" }}>
          <div style={{ background: "white", borderRadius: "20px", padding: "32px", width: "100%", maxWidth: "500px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <input type="text" placeholder="Search by name or phone..." value={patientSearch} onChange={e => setPatientSearch(e.target.value)}
              style={{ width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1.5px solid #e0e7ff", fontSize: "15px", fontFamily: "Georgia, serif", boxSizing: "border-box", marginBottom: "12px" }} />
            <div style={{ maxHeight: "280px", overflowY: "auto", border: "1px solid #e0e7ff", borderRadius: "10px", marginBottom: "20px" }}>
              {patients.filter(p => p.name.toLowerCase().includes(patientSearch.toLowerCase()) || p.phone.includes(patientSearch)).map(p => (
                <div key={p.patient_id} onClick={() => setSelectedPatient(p.patient_id)}
                  style={{ padding: "12px 16px", cursor: "pointer", borderBottom: "1px solid #f0f0f0", background: selectedPatient === p.patient_id ? "#f0f4ff" : "white", display: "flex", justifyContent: "space-between" }}>
                  <div>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button onClick={() => { setShowAssign(false); setSelectedPatient(null); setPatientSearch(""); }} style={{ flex: 1, background: "#f0f4ff", color: "#0a2463", border: "none", padding: "12px", borderRadius: "10px", fontSize: "15px", fontWeight: "600", cursor: "pointer" }}>Cancel</button>
              <button onClick={assignRoom} disabled={saving || !selectedPatient} style={{ flex: 1, background: saving || !selectedPatient ? "#94a3b8" : "#0a2463", color: "white", border: "none", padding: "12px", borderRadius: "10px", fontSize: "15px", fontWeight: "700", cursor: saving || !selectedPatient ? "not-allowed" : "pointer" }}>
      ...
      {showMedicines && selectedRoom && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "20px" }}>
          <div style={{ background: "white", borderRadius: "20px", padding: "32px", width: "100%", maxWidth: "560px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", maxHeight: "90vh", overflowY: "auto" }}>
            <h3 style={{ color: "#0a2463", fontSize: "20px", marginBottom: "4px" }}>💊 Medicines Given</h3>
            <p style={{ color: "#666", fontSize: "14px", marginBottom: "20px" }}>Room {selectedRoom.room_number} · {(selectedRoom.patient as any)?.name}</p>
            <div style={{ background: "#f8f9fc", borderRadius: "12px", padding: "16px", marginBottom: "20px", border: "1px solid #e8edf5" }}>
              <div style={{ fontWeight: "600", fontSize: "13px", color: "#0a2463", marginBottom: "12px" }}>Add Medicine</div>
              <div style={{ position: "relative", marginBottom: "10px" }}>
                <input value={medSearch} onChange={e => handleMedSearch(e.target.value)} placeholder="Type 2+ letters to search..."
                  style={{ width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1.5px solid #e0e7ff", fontSize: "14px", fontFamily: "Georgia, serif", boxSizing: "border-box" }} />
                {selectedMedName && <div style={{ fontSize: "12px", color: "#16a34a", marginTop: "4px", fontWeight: "600" }}>✓ {selectedMedName}</div>}
                {medSuggestions.length > 0 && !selectedMedId && (
                  <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "white", borderRadius: "8px", border: "1.5px solid #e0e7ff", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", zIndex: 50, maxHeight: "180px", overflowY: "auto" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "10px", marginBottom: "10px" }}>
                <div>
                  <label style={{ fontSize: "12px", color: "#666", display: "block", marginBottom: "4px" }}>Qty</label>
                  <input type="number" value={medQty} onChange={e => setMedQty(e.target.value)} min="1"
                    style={{ width: "100%", padding: "8px 10px", borderRadius: "6px", border: "1px solid #e0e7ff", fontSize: "14px", fontFamily: "Georgia, serif", boxSizing: "border-box" }} />
                </div>
                <div>
                  <label style={{ fontSize: "12px", color: "#666", display: "block", marginBottom: "4px" }}>Notes (optional)</label>
                  <input value={medNotes} onChange={e => setMedNotes(e.target.value)} placeholder="e.g. IV, oral..."
                    style={{ width: "100%", padding: "8px 10px", borderRadius: "6px", border: "1px solid #e0e7ff", fontSize: "14px", fontFamily: "Georgia, serif", boxSizing: "border-box" }} />
                </div>
              </div>
              <button onClick={addMedicineToPatient} disabled={saving || !selectedMedId}
                style={{ width: "100%", padding: "10px", background: saving || !selectedMedId ? "#94a3b8" : "#7c3aed", color: "white", border: "none", borderRadius: "8px", fontSize: "14px", fontWeight: "700", cursor: saving || !selectedMedId ? "not-allowed" : "pointer" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "20px" }}>
                {medicinesUsed.map((m, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "#f8f9fc", borderRadius: "8px", border: "1px solid #e8edf5" }}>
                    <div>
                      <div style={{ fontWeight: "600", color: "#0a2463", fontSize: "13px" }}>{m.name}</div>
                      <div style={{ color: "#888", fontSize: "11px" }}>Qty: {m.quantity} · ₹{m.unit_price} each = <strong>₹{(m.unit_price * m.quantity).toFixed(2)}</strong>{m.notes ? ` · ${m.notes}` : ""}</div>
                    </div>
                    <button onClick={() => removeMedicine(m.pres_id)} style={{ background: "#fee2e2", color: "#dc2626", border: "none", padding: "4px 10px", borderRadius: "6px", fontSize: "12px", cursor: "pointer" }}>Remove</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {showBill && bill && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "20px" }}>
          <div style={{ background: "white", borderRadius: "20px", padding: "32px", width: "100%", maxWidth: "480px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", maxHeight: "90vh", overflowY: "auto" }}>
            <h3 style={{ color: "#0a2463", fontSize: "20px", marginBottom: "4px" }}>Bill Summary</h3>
            <p style={{ color: "#666", fontSize: "14px", marginBottom: "24px" }}>Room {bill.room.room_number} · {bill.days} day{bill.days > 1 ? "s" : ""}</p>
            {[
            ...
            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginTop: "8px" }}>
              <button onClick={() => { setShowBill(false); setShowDischarge(false); setBill(null); }} style={{ flex: 1, background: "#f0f4ff", color: "#0a2463", border: "none", padding: "12px", borderRadius: "10px", fontSize: "15px", fontWeight: "600", cursor: "pointer" }}>Close</button>
              <button onClick={handlePrintBill} style={{ flex: 1, background: "white", color: "#0a2463", border: "2px solid #0a2463", padding: "12px", borderRadius: "10px", fontSize: "15px", fontWeight: "700", cursor: "pointer" }}>🖨️ Print Bill</button>
              {showDischarge && (
                <button onClick={dischargePatient} disabled={saving} style={{ width: "100%", background: saving ? "#94a3b8" : "#dc2626", color: "white", border: "none", padding: "12px", borderRadius: "10px", fontSize: "15px", fontWeight: "700", cursor: saving ? "not-allowed" : "pointer" }}>
