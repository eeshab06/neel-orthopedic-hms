...
const SURGERY_TYPES = [
  ...
  "Fracture Fixation - Hip (PFNA)",
  "Fracture Fixation - Forearm",
  "Ankle Fusion",
  "Arthroscopy - Knee",
...
interface Surgery {
  ...
  surgery_type: string;
  ot_number: string;
  anaesthesia_dr: string;
  assistant_dr: string;
  implant_id: number | null;
  status: string;
  notes: string | null;
  physio_booked: boolean;
  cardio_cleared: boolean;
...
export default function IPDPage() {
  ...
  const [form, setForm] = useState({
    ...
    surgery_time: "08:00",
    surgery_type: "", ot_number: "OT-1",
    anaesthesia_dr: "Dr. Vijay Rangani",
    assistant_dr: "", implant_id: "", notes: "",
  });
  ...
  const getDaysSince = (d: string) =>
    Math.ceil((Date.now() - new Date(d).getTime()) / (1000 * 60 * 60 * 24));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f4ff", fontFamily: "Georgia, serif" }}>
      ...
      {/* IPD Header */}
      <div style={{ background: "#0a2463", padding: "16px 32px" }}>
        <h1 style={{ color: "#fff", margin: 0, fontSize: 20, fontWeight: 700 }}>🏥 IPD & Surgery Management</h1>
        <p style={{ color: "#a8c0ff", margin: "2px 0 0", fontSize: 13 }}>Neel Orthopaedic Hospital — Operation Theatre & Inpatient</p>
      </div>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 16, marginBottom: 24 }}>
          {[
            { label: "Today's Surgeries", value: todaySurgeries, color: "#0a2463", icon: "📅" },
            { label: "Scheduled",         value: scheduled,      color: "#1a73e8", icon: "🗓️" },
            { label: "In Progress",       value: inProgress,     color: "#d97706", icon: "⚡" },
            { label: "Completed",         value: completed,      color: "#16a34a", icon: "✅" },
            { label: "Current Inpatients",value: inpatients.length, color: "#7c3aed", icon: "🛏️" },
          ].map(s => (
        ...
        <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 2px 8px rgba(10,36,99,0.07)" }}>
          <div style={{ display: "flex", borderBottom: "2px solid #e8eef8", padding: "0 16px" }}>
            {([
              ...
              <button key={t.key} onClick={() => setActiveTab(t.key)} style={{ padding: "16px 22px", border: "none", background: "transparent", fontFamily: "Georgia, serif", fontSize: 14, fontWeight: activeTab === t.key ? 700 : 400, color: activeTab === t.key ? "#0a2463" : "#666", borderBottom: activeTab === t.key ? "3px solid #0a2463" : "3px solid transparent", cursor: "pointer", marginBottom: -2, whiteSpace: "nowrap" }}>
                {t.label}
              </button>
            ))}
          </div>
            ...
            <div style={{ padding: 24 }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: "center", padding: 60, color: "#aaa" }}>
                  ...
                  <p>No surgeries found. Use Schedule Surgery tab to add one.</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {filteredSurgeries.map(s => {
                    ...
                    return (
                      <div key={s.surgery_id} style={{ background: isToday ? "#f0f9ff" : "#fafafa", border: `1.5px solid ${isToday ? "#bfdbfe" : "#e8eef8"}`, borderRadius: 12, padding: "18px 20px", display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
                        {/* Left — main info */}
                        <div style={{ flex: 1, minWidth: 200 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
                            <span style={{ fontWeight: 700, color: "#0a2463", fontSize: 16 }}>{s.patient?.name || "—"}</span>
                            <span style={{ fontSize: 12, color: "#888" }}>UHID: {s.patient?.uhid || "—"}</span>
                            {isToday && <span style={{ background: "#fef9c3", color: "#854d0e", fontSize: 11, fontWeight: 700, borderRadius: 6, padding: "2px 8px" }}>TODAY</span>}
                          </div>
                          <div style={{ fontSize: 14, color: "#333", fontWeight: 600, marginBottom: 4 }}>{s.surgery_type}</div>
                          ...
                          {s.anaesthesia_dr && <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>Anaesthetist: {s.anaesthesia_dr}</div>}
                          {s.notes && <div style={{ fontSize: 12, color: "#666", marginTop: 4, fontStyle: "italic" }}>📝 {s.notes}</div>}
                          {/* Team notification buttons */}
                          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                            {Object.entries(TEAM).map(([key, t]) => (
                              <a key={key} href={waLink(t.phone, buildWAMessage(s, t.name))} target="_blank" rel="noreferrer"
                                style={{ background: "#25D366", color: "#fff", textDecoration: "none", borderRadius: 6, padding: "5px 12px", fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}>
                                💬 {t.role}
                              </a>
                            ))}
                          </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, minWidth: 160 }}>
                          ...
                          <select value={s.status} disabled={updatingId === s.surgery_id}
                            ...
                            style={{ padding: "7px 10px", border: "1px solid #dbeafe", borderRadius: 7, fontSize: 12, cursor: "pointer", fontFamily: "Georgia, serif", background: "#fff" }}>
                            <option>Scheduled</option>
                            <option>In Progress</option>
                            <option>Completed</option>
                            <option>Cancelled</option>
                          </select>
                          <div style={{ fontSize: 11, color: "#aaa" }}>#{s.surgery_id}</div>
                        </div>
                      </div>
                    );
                  })}
          ...
          {activeTab === "schedule" && (
            <div style={{ padding: 32, maxWidth: 860 }}>
              <h3 style={{ margin: "0 0 6px", color: "#0a2463", fontSize: 20 }}>Schedule New Surgery</h3>
              <p style={{ margin: "0 0 28px", color: "#888", fontSize: 13 }}>Fill in all details. WhatsApp notifications will be available after scheduling.</p>

              {/* Patient search */}
              <div style={{ marginBottom: 20, position: "relative" }}>
                <label style={lbl}>Patient * — Search by name or UHID</label>
                {selectedPatient ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 12, background: "#f0f9ff", border: "1.5px solid #1a73e8", borderRadius: 8, padding: "12px 16px" }}>
                    <div>
                      <div style={{ fontWeight: 700, color: "#0a2463" }}>{selectedPatient.name}</div>
                      <div style={{ fontSize: 12, color: "#666" }}>UHID: {selectedPatient.uhid} | 📞 {selectedPatient.phone}</div>
                    </div>
                    <button onClick={() => { setSelectedPatient(null); setPatientSearch(""); setForm(f => ({ ...f, patient_id: "" })); }}
                      style={{ marginLeft: "auto", background: "#fee2e2", color: "#dc2626", border: "none", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 12 }}>Change</button>
                  </div>
                ) : (
                  <>
                    ...
                    {patientResults.length > 0 && (
                      <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #dbeafe", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.1)", zIndex: 100, marginTop: 4 }}>
                        {patientResults.map(p => (
                          <div key={p.patient_id} onClick={() => { setSelectedPatient(p); setForm(f => ({ ...f, patient_id: String(p.patient_id) })); setPatientSearch(""); setPatientResults([]); }}
                            style={{ padding: "10px 16px", cursor: "pointer", borderBottom: "1px solid #f0f4ff" }}
                            onMouseEnter={e => (e.currentTarget.style.background = "#f0f9ff")}
                            onMouseLeave={e => (e.currentTarget.style.background = "#fff")}>
                            <span style={{ fontWeight: 600, color: "#0a2463" }}>{p.name}</span>
                            <span style={{ fontSize: 12, color: "#888", marginLeft: 8 }}>UHID: {p.uhid}</span>
                            <span style={{ fontSize: 12, color: "#888", marginLeft: 8 }}>📞 {p.phone}</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <button onClick={handleSchedule} disabled={submitting}
                style={{ background: submitting ? "#94a3b8" : "#0a2463", color: "#fff", border: "none", borderRadius: 10, padding: "14px 40px", fontSize: 16, fontWeight: 600, cursor: submitting ? "not-allowed" : "pointer", fontFamily: "Georgia, serif" }}>
                {submitting ? "Scheduling…" : "📅 Schedule Surgery"}
              </button>

              {/* WhatsApp notification panel — shows after scheduling */}
              {formSuccess && scheduledSurgery && (
                <div style={{ marginTop: 28, background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 12, padding: 24 }}>
                  ...
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    {Object.entries(TEAM).map(([key, t]) => (
                      <a key={key} href={waLink(t.phone, buildWAMessage(scheduledSurgery, t.name))} target="_blank" rel="noreferrer"
                        style={{ background: "#25D366", color: "#fff", textDecoration: "none", borderRadius: 10, padding: "12px 20px", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}>
                        💬 Notify {t.name}
                        <span style={{ fontSize: 11, opacity: 0.85 }}>({t.role})</span>
                      </a>
                    ))}
                  </div>
                  <p style={{ fontSize: 12, color: "#888", marginTop: 12 }}>Each button opens WhatsApp with a pre-filled message. Just tap Send.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "inpatients" && (
            <div style={{ padding: 24 }}>
              <h3 style={{ margin: "0 0 20px", color: "#0a2463", fontSize: 18 }}>Current Inpatients</h3>
              {loading ? (
                <div style={{ textAlign: "center", padding: 60, color: "#888" }}>Loading…</div>
              ) : inpatients.length === 0 ? (
                <div style={{ textAlign: "center", padding: 60, color: "#aaa" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
                  {inpatients.map(ipd => {
                    ...
                    return (
                      <div key={ipd.ipd_id} style={{ background: "#fff", border: "1.5px solid #e8eef8", borderRadius: 12, padding: 20 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                          <div>
                            <div style={{ fontWeight: 700, color: "#0a2463", fontSize: 16 }}>{ipd.patient?.name || "—"}</div>
                            <div style={{ fontSize: 12, color: "#888" }}>UHID: {ipd.patient?.uhid || "—"}</div>
                          </div>
                          <span style={{ background: "#dcfce7", color: "#16a34a", borderRadius: 6, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>ADMITTED</span>
                        </div>
                        <div style={{ fontSize: 13, color: "#555", display: "flex", flexDirection: "column", gap: 4 }}>
                          <span>🛏️ Room: <strong>{ipd.room_number}</strong></span>
                          <span>📅 Admitted: {ipd.admit_date}</span>
                          <span>⏱️ Day <strong>{days}</strong> of stay</span>
                          {ipd.patient?.phone && <span>📞 {ipd.patient.phone}</span>}
                        </div>
                        {linkedSurgery && (
                          <div style={{ marginTop: 12, background: "#f0f9ff", borderRadius: 8, padding: "10px 12px", border: "1px solid #bfdbfe" }}>
                            <div style={{ fontSize: 12, fontWeight: 700, color: "#1e40af", marginBottom: 4 }}>🔬 Surgery Scheduled</div>
                            <div style={{ fontSize: 12, color: "#333" }}>{linkedSurgery.surgery_type}</div>
                            <div style={{ fontSize: 11, color: "#666" }}>{linkedSurgery.surgery_date} {linkedSurgery.surgery_time ? `at ${linkedSurgery.surgery_time.slice(0, 5)}` : ""} · {linkedSurgery.ot_number}</div>
                            <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
                              {Object.entries(TEAM).map(([key, t]) => (
                                <a key={key} href={waLink(t.phone, buildWAMessage(linkedSurgery, t.name))} target="_blank" rel="noreferrer"
                                  style={{ background: "#25D366", color: "#fff", textDecoration: "none", borderRadius: 5, padding: "3px 8px", fontSize: 10, fontWeight: 700 }}>
                                  💬 {t.role.split(" ")[0]}
                                </a>
                              ))}
                            </div>
